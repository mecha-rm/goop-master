...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(300, 300, "GSD - TRL01", nullptr, nullptr);
	...
	// if the file isn't found, nothing happens.
	audioEngine.LoadBank("Master", FMOD_STUDIO_LOAD_BANK_NORMAL); // makes sure the bank is loaded before the audio starts.

	//// Load an event
	// the GUID comes from FMod Studio, and is unique for every event. This is how the code knows which event to play.
	audioEngine.LoadEvent("Music", "{87f3decc-8e01-48a5-8868-9c3c1fa48e87}");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (TTK::Input::GetKeyPressed(TTK::KeyCode::Q)) {
			...
			if (isOnYoshi) {
				...
				//// Set isOnYoshi Parameter to 1 
				audioEngine.SetEventParameter("Music", "isYoshi", 1.0F);
				currentSprite = &yoshi;
			}
			else {
				
				//// Set isOnYoshi Parameter to 0
				audioEngine.SetEventParameter("Music", "isYoshi", 0.0F);
				currentSprite = &mario;
			}
		}
	...
	audioEngine.Shutdown(); // shut-down to avoid memory leaks.
